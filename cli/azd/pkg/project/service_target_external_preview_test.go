...
func newExternalPreviewTarget(
	...
	target := NewExternalServiceTargetWithPreviewSupport(
...
func TestExternalServiceTargetPreviewFailsClosed(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			target: NewExternalServiceTarget("custom", "custom", nil, nil, nil, nil, nil),
		},
		{
			name:   "ExplicitlyUnsupported",
			target: NewExternalServiceTargetWithPreviewSupport("custom", "custom", nil, nil, nil, nil, nil, false),
		},
	} {
...
func TestExternalServiceTargetPreviewUsesOnlyPreviewRequest(t *testing.T) {
	...
	properties := map[string]any{
		"$ref": "${AGENT_DEFINITION_PATH}",
		"environment_variables": map[string]any{
			"$ref": "environment.yaml",
		},
	}
	service := &ServiceConfig{
		Name: "api", Host: "custom", Image: osutil.NewExpandableString("${IMAGE_NAME}"),
	...
	require.Equal(t, properties, request.GetPreviewRequest().GetServiceConfig().AdditionalProperties.AsMap(),
		"preserve source references for the provider's modern-versus-legacy validation")
	...
	require.Equal(t, properties, service.AdditionalProperties)
}
...
func TestExternalServiceTargetPreviewErrors(t *testing.T) {
	...
	t.Run("Provider", func(t *testing.T) {
		...
		azdext.NewServiceTargetEnvelope().SetError(response, errors.New("provider read failed"))
	...
	t.Run("Environment", func(t *testing.T) {
		t.Parallel()
		target, stream := newExternalPreviewTarget(t, nil, nil)
		expected := errors.New("environment read failed")
		target.lazyEnv = lazy.NewLazy(func() (*environment.Environment, error) { return nil, expected })
		_, err := target.Preview(t.Context(), &ServiceConfig{Name: "api"})
