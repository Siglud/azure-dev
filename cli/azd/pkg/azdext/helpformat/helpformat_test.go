...
func TestUsagePreservesSDKOverridesAndWriter(t *testing.T) {
	setColor(t, false)
	for _, mode := range []string{"help", "usage", "usage string"} {
		t.Run(mode, func(t *testing.T) {
			root, _ := azdext.NewExtensionRootCommand(azdext.ExtensionCommandOptions{Name: "example"})
			child := &cobra.Command{Use: "show", Run: func(*cobra.Command, []string) {}}
			root.AddCommand(child)
			azdext.RegisterFlagOptions(child, azdext.FlagOptions{
				Name: "output", Default: "json", AllowedValues: []string{"json", "table"},
				Usage: "Choose a format", HideDefault: true,
			})
			flag := root.PersistentFlags().Lookup("output")
			originalUsage, originalDefault := flag.Usage, flag.DefValue
			Install(root, "azd", "")
			...
			case "usage":
				...
				require.Empty(t, errOut.String())
			case "usage string":
				text = child.UsageString()
				require.Empty(t, out.String())
				require.Empty(t, errOut.String())
			}
			require.Contains(t, text, "azd example show [flags]")
			require.Contains(t, text, "Choose a format")
			require.Contains(t, text, "supported: json, table")
			require.NotContains(t, text, `(default "json")`)
			require.Equal(t, originalUsage, flag.Usage)
			require.Equal(t, originalDefault, flag.DefValue)
			require.NotContains(t, root.UsageString(), "Choose a format")
		})
	}
}
