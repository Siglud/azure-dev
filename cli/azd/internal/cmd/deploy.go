...
func (d *DeployFlags) Bind(local *pflag.FlagSet, global *internal.GlobalCommandOptions) {
	...
	local.BoolVar(
		...
		"preview",
		false,
		"Previews unified hosted Foundry agent deployment without building, publishing, or deploying.",
	)
}
...
func NewDeployAction(
	...
	if flags.Preview {
		if err := flags.validatePreview(args); err != nil {
			return nil, err
		}
...
func GetCmdDeployHelpDescription(*cobra.Command) string {
	return generateCmdHelpDescription("Deploy application to Azure.", []string{
		...
		formatHelpNote("Use --preview for hosted Foundry agents (host: azure.ai.agent) defined in the unified" +
			" service-level azure.yaml format, without running hooks, building, publishing, or deploying." +
			" Legacy agent.yaml/agent.yml and nested config: definitions are unsupported;" +
			" move the agent definition to the unified azure.yaml format to use preview." +
...
func GetCmdDeployHelpFooter(*cobra.Command) string {
	return generateCmdHelpSamplesBlock(map[string]string{
		...
		"Preview deployment of the unified hosted Foundry agent service named 'api'.": output.WithHighLightFormat(
