...
func (d *DeployFlags) validatePreview(args []string) error {
	if d.fromPackage != "" || (d.flagSet != nil && d.flagSet.Changed("from-package")) {
		return errors.New("cannot specify both --preview and --from-package")
	}
	if len(args) != 0 && d.flagSet != nil && d.flagSet.Changed("service") {
		return errors.New("cannot specify both <service> and --service with --preview")
	}
	if d.All && (len(args) != 0 || d.ServiceName != "" || (d.flagSet != nil && d.flagSet.Changed("service"))) {
...
func (da *DeployAction) preview(ctx context.Context, targetServiceName string) (*actions.ActionResult, error) {
	if err := da.flags.validatePreview(da.args); err != nil {
		return nil, err
	}
	...
	if da.formatter.Kind() == output.JsonFormat {
		...
		for _, svc := range services {
			...
			if !exists {
				if _, err := fmt.Fprintf(da.writer,
					"  (-) Skipped: service '%s' (host: %s) does not support deployment preview.\n",
					sanitizeServiceName(svc.Name), sanitizeServiceName(string(svc.Host)),
				); err != nil {
					return nil, fmt.Errorf("deployment preview could not be displayed: %w", err)
				}
				continue
