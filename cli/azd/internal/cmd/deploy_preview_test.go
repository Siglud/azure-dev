...
func TestDeployPreviewRejectsConflictingFlags(t *testing.T) {
	tests := []struct {
		...
		{name: "AllAndEmptyService", args: []string{"--all", "--service="}, want: "--all and <service>"},
		{name: "PositionalAndService", args: []string{"api", "--service", "web"}, want: "<service> and --service"},
		{name: "PositionalAndEmptyService", args: []string{"api", "--service="}, want: "<service> and --service"},
